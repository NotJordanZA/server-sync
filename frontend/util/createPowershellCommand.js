...
export async function constructRunSyncCommand(profileName, syncNow = false){
    const runSyncPath = path.join(__dirname,  "runSync.js");
    const baseCommand = `node "${runSyncPath}"`;
    const profile = await loadProfileWithPath(`./syncProfiles/${profileName}.txt`);
